...
Struktly is pre-1.0 and **experimental**, but its formats are versioned from v0.4 onward. This document is the contract.
...
- **CLI surface**: existing commands and flags are stable and only extended additively. New surfaces launch labeled *Experimental* in `--help` and the README, and may change without a major-version signal until the label is removed.
...
The complete command/stream/exit/security contract is
[docs/integration-contract.md](integration-contract.md).

## Status labels

Capabilities are labeled **Released** (stable per the rules above) or
**Experimental** (implemented, but its command surface may change). The README
carries the current table; it does not advertise unimplemented work.
