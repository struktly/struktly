...
func TestRecordEvidenceAppendsStructuredEntry(t *testing.T) {
	...
	result, err := RecordEvidence(EvidenceOptions{
		Root:          root,
		Task:          "Record verification evidence",
		Agent:         "Cursor agent",
		Outcome:       "Implemented append-only evidence writer with tests.",
		ContextPacket: packetPath,
		Checks:        []string{"go test ./internal/context/..."},
		CheckResult:   "pass",
		FilesTouched:  []string{"service/internal/context/evidence.go"},
		Reviewer:      "pending",
	...
	content = string(data)
	if strings.Count(content, "## 2026-07-05T12:00:00Z — Record verification evidence") != 1 {
...
func TestRecordEvidenceRunChecksRejectsShellSyntax(t *testing.T) {
	root := t.TempDir()
	result, err := RecordEvidence(EvidenceOptions{
		Root:      root,
		Task:      "Verified fail",
...
func TestRecordEvidenceRunChecksFailsWithBadCommand(t *testing.T) {
	root := t.TempDir()
	result, err := RecordEvidence(EvidenceOptions{
		Root:      root,
		Task:      "Long output",
	...
	content := string(data)
	assertContains(t, content, "- `false` — fail (exit 1,")
	if strings.Contains(content, "line6") {
		t.Fatalf("expected no shell-loop output artifacts in the check command:\n%s", content)
	}
}
...
func TestRecordEvidenceRunChecksTimesOut(t *testing.T) {
	...
	root := t.TempDir()
	writeFile(t, root, "sleeper/main.go", `
package main
...
`)
	result, err := RecordEvidence(EvidenceOptions{
		...
		Agent:     "struktly",
		Outcome:   "Checks executed.",
		Checks:    []string{"go run ./sleeper/main.go"},
		RunChecks: true,
	})
	if err != nil {
		t.Fatalf("RecordEvidence returned error: %v", err)
