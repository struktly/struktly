...
const (
	evidenceLedgerLockTimeout  = 10 * time.Second
	evidenceLedgerLockStaleAge = 30 * time.Second
)
...
func RecordEvidence(opts EvidenceOptions) (EvidenceResult, error) {
	...
	if err := appendEvidenceEntry(outputPath, now, entry); err != nil {
		return EvidenceResult{}, err
	}
...
func executeCheck(root, command string) checkRun {
	parts, parseErr := parseCheckCommand(command)
	if parseErr != nil {
		return checkRun{
			Command: command,
			Status:  fmt.Sprintf("fail (invalid command: %v)", parseErr),
			Tail:    []string{parseErr.Error()},
		}
	}
...
func parseCheckCommand(command string) ([]string, error) {
	command = strings.TrimSpace(command)
	if command == "" {
		return nil, fmt.Errorf("command is required")
	}
	if strings.ContainsAny(command, "\n\r\t") {
		return nil, fmt.Errorf("command contains unsupported whitespace")
	}
	if strings.ContainsAny(command, "&|;<>$`()") {
		return nil, fmt.Errorf("command uses shell metacharacters")
	}
...
func appendEvidenceEntry(outputPath string, now time.Time, entry string) error {
	if err := withEvidenceLedgerLock(outputPath, func() error {
		existingInfo, statErr := os.Stat(outputPath)
		if statErr != nil && !os.IsNotExist(statErr) {
			return fmt.Errorf("stat evidence ledger: %w", statErr)
		}
		...
		if _, err := f.WriteString(entry); err != nil {
			return fmt.Errorf("write evidence ledger entry: %w", err)
		}
		return nil
	}); err != nil {
...
func withEvidenceLedgerLock(outputPath string, fn func() error) error {
	lockPath := outputPath + ".lock"
	deadline := time.Now().Add(evidenceLedgerLockTimeout)
	for {
		...
		if err == nil {
			...
			fnErr := fn()
			if removeErr := os.Remove(lockPath); removeErr != nil && !os.IsNotExist(removeErr) && fnErr == nil {
				return removeErr
			}
			return fnErr
		}
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("timeout while waiting for evidence ledger lock")
		}
		if info, statErr := os.Stat(lockPath); statErr == nil {
			if time.Since(info.ModTime()) > evidenceLedgerLockStaleAge {
