...
func Scan(opts ScanOptions) (ScanResult, error) {
	...
	content := files.OKFFrontmatter("project-context", "Project Context: "+scan.name, "Deterministic repository context generated by struktly scan.", now) + scan.renderMarkdown()
	...
	if rel == ".struktly/direction.md" {
		...
		if text, err := files.ReadSmallTextFile(path, maxSummaryFileBytes); err == nil {
			if containsSecret(text) {
				...
				s.productDirection = excerptMarkdown(files.StripFrontmatter(text), 1200)
			}
		} else {
...
func (s *repositoryScan) finalizeOpenQuestions() {
	if s.productDirection == "" {
		s.openQuestions = append(s.openQuestions, "Missing `.struktly/direction.md`; add one to anchor product direction for agent sessions.")
	}
	if _, err := os.Stat(filepath.Join(s.root, ".struktly", "constraints.md")); err != nil {
		s.openQuestions = append(s.openQuestions, "No `.struktly/constraints.md` file found; add one to pin constraints and non-goals for agent sessions.")
	}
	if len(s.adrs) == 0 {
		s.openQuestions = append(s.openQuestions, "No ADR or decision docs found.")
	}
	if len(s.agentFiles) == 0 {
		s.openQuestions = append(s.openQuestions, "No agent instruction files found (`AGENTS.md`, `CLAUDE.md`, Cursor rules, or Copilot instructions).")
	}
	if _, err := os.Stat(filepath.Join(s.root, ".struktly", "decisions.md")); err != nil {
		s.openQuestions = append(s.openQuestions, "No `.struktly/decisions.md` ledger found yet.")
	}
	if _, err := os.Stat(filepath.Join(s.root, ".struktly", "evidence.md")); err != nil {
		s.openQuestions = append(s.openQuestions, "No `.struktly/evidence.md` ledger found yet.")
	}
	if len(s.commands) == 0 {
...
func (s *repositoryScan) renderMarkdown() string {
	...
	b.WriteString("# Struktly Project Context\n\n")
	b.WriteString("Generated by `struktly scan` using deterministic filesystem heuristics. No model calls were made.\n\n")
	...
	b.WriteString("## Detected Languages / Frameworks\n\n")
	...
	b.WriteString("## Existing ADRs / Decision Docs\n\n")
	writePathBullets(&b, files.SortedStrings(s.adrs), "No ADR or decision docs detected.")
	b.WriteString("\n")

	b.WriteString("## Existing Agent Instruction Files\n\n")
	writePathBullets(&b, files.SortedStrings(s.agentFiles), "No agent instruction files detected.")
	b.WriteString("\n")

	b.WriteString("## Sensitive / Ignored Paths\n\n")
	writeBullets(&b, []string{
		"Default ignored directories: `" + strings.Join(files.DefaultIgnoredDirs, "`, `") + "`",
		"Default ignored generated Struktly paths: `" + strings.Join(files.DefaultIgnoredPaths, "`, `") + "`",
		"`.env` / `.env.*`",
		"`*.pem`, `*.key`, `*.p12`, `*.pfx`",
	...
	if len(s.stale) > 0 {
		...
		b.WriteString("These directories look stale or fixture-only (names like `legacy`, `archive`, `deprecated`, `old`, `testdata`) and were skipped when collecting docs, commands, and languages:\n\n")
		writePathBullets(&b, files.LimitStrings(files.SortedStrings(s.stale), 20), "")
		b.WriteString("\n")
	}

	if s.productDirection != "" {
		b.WriteString("## Known Product Direction\n\n")
		b.WriteString("Source: `" + s.productSourcePath + "`\n\n")
		b.WriteString(s.productDirection)
		b.WriteString("\n\n")
	}

	b.WriteString("## Open Questions / Missing Metadata\n\n")
	writeBullets(&b, s.openQuestions, "No obvious missing metadata detected.")
	b.WriteString("\n")
