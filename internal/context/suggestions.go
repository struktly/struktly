...
func (d *instructionDraft) readOptionalInputs() {
	...
	commands := rankCommands(extractBullets(sectionContent(d.projectContext, "## Build / Test Commands")))
...
func (d *instructionDraft) renderMarkdown(target string) string {
	...
	b.WriteString(files.OKFFrontmatter("agent-instructions", "Struktly Agent Instructions ("+target+")", "Draft agent instruction file generated by struktly suggest-instructions.", d.generatedAt))
	b.WriteString("# Struktly Agent Instructions (Suggested)\n\n")
	b.WriteString("- Generated: `" + d.generatedAt.Format(time.RFC3339) + "`\n")
	b.WriteString("- Mode: local-first deterministic suggestion; no model calls.\n")
	b.WriteString("- Status: draft suggestion only — review before promotion.\n\n")
	...
	if strings.TrimSpace(d.constraints) != "" {
		...
		b.WriteString("No `.struktly/constraints.md` file was found. Create it to pin constraints and non-goals for agent sessions.\n\n")
	}

	if len(nonGoals) > 0 {
		b.WriteString("## Product Non-goals\n\n")
		b.WriteString("From `.struktly/direction.md` — do not build or assume any of these:\n\n")
	...
	b.WriteString("## Struktly Workflow\n\n")
